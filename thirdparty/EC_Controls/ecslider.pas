...
begin
  ...
  with GrooveBMP.Canvas do
    begin
      ...
      if IsColorDark(clBtnFace)
        then aColor2:=GetColorResolvingDefault(ProgressColor2, clInactiveCaption)
        else aColor2:=GetColorResolvingDefault(ProgressColor2, clHighlightText);
...
begin
  ...
  if not AMove then exclude(Flags, esfPrevInvRectPainted);
end;
...
begin
  ...
  Result:=(aReversed<>(esfRealReversed in Flags));
  if Result then
    begin
