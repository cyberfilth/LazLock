...
begin
  ...
  if MenuItems.Count > 0 then
    begin
      ...
      if aCaption <> '' then
        begin
          aRect := ThemeServices.GetTextExtent(Canvas.Handle, aDetails, aCaption, 0, nil);
          aDescHeight := aRect.Bottom;  { aBMP.Canvas.TextHeight(aCaption); }
        end;
  ...
  if MenuItems[Index].Description <> '' then
    begin
      ...
      ThemeServices.DrawText(Canvas, aDetails, MenuItems[Index].Description, 
        aTextRect, aFlags or DT_NOPREFIX or DT_VCENTER, 0);
